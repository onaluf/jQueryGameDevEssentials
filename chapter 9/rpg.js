...
$(function() {
	...
	gf.initialize({baseDiv: $("#mygame"), width: 480, height: 320});
    ...
    var gameLoop = function() {
        ...
        var margin = {x: (480-192)/2, y: (320-192)/2}; 
        var playerPos = {x: gf.x(player.div), y: gf.y(player.div)};
        
        gf.x(world, margin.x-Math.min(Math.max(playerPos.x, margin.x), 2048-480+margin.x));
        gf.y(world, margin.y-Math.min(Math.max(playerPos.y, margin.y), 2048-320+margin.y));
